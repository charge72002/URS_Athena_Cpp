...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // parse shock 1 location (must be inside grid)
  Real xshock1 = pin->GetReal("problem","xshock1");
  if (shk_dir == 1 && (xshock1 < pmy_mesh->mesh_size.x1min ||
                       xshock1 > pmy_mesh->mesh_size.x1max)) {
    msg << "### FATAL ERROR in Problem Generator" << std::endl << "xshock1="
        << xshock1 << " lies outside x1 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }
  if (shk_dir == 2 && (xshock1 < pmy_mesh->mesh_size.x2min ||
                       xshock1 > pmy_mesh->mesh_size.x2max)) {
    msg << "### FATAL ERROR in Problem Generator" << std::endl << "xshock1="
        << xshock1 << " lies outside x2 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }
  if (shk_dir == 3 && (xshock1 < pmy_mesh->mesh_size.x3min ||
                       xshock1 > pmy_mesh->mesh_size.x3max)) {
    msg << "### FATAL ERROR in Problem Generator" << std::endl << "xshock1="
        << xshock1 << " lies outside x3 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }

   //parse in bump location
   Real xshock2 = pin->GetReal("problem","xshock2");
   Real xshock3 = pin->GetReal("problem","xshock3");
...
/*
  // Parse bump state read from input file: db,ub,vb,wb,[pb]
  Real wb[NHYDRO+NFIELD];
  wb[IDN] = pin->GetReal("problem","db");
  wb[IVX] = pin->GetReal("problem","ub");
  wb[IVY] = pin->GetReal("problem","vb");
  wb[IVZ] = pin->GetReal("problem","wb");
  if (NON_BAROTROPIC_EOS) {
    if (pin->DoesParameterExist("problem","Tb"))
      wb[IPR] = press(wl[IDN], pin->GetReal("problem","Tb"));
    else
      wb[IPR] = pin->GetReal("problem","pb");
  }
  if (MAGNETIC_FIELDS_ENABLED) {
    wl[NHYDRO  ] = pin->GetReal("problem","bxb");
    wl[NHYDRO+1] = pin->GetReal("problem","byb");
    wl[NHYDRO+2] = pin->GetReal("problem","bzb");
  }
*/
      ...
      for (int k=ks; k<=ke; ++k) {
        for (int j=js; j<=je; ++j) {
          for (int i=is; i<=ie; ++i) {
            ...
            //(part of) SHERRY'S MODIFICATION STARTS HERE
            } else if (xshock2 < pcoord->x1v(i) && pcoord->x1v(i) < xshock3){
              phydro->u(IDN,k,j,i) = sin(pcoord->x1v(i));
